/**
 * BEE Keepers MVP - Google Apps Script REST API (WORKING CORS VERSION)
 * 
 ...
 * 
 * CORS FIX: Using proper Google Apps Script CORS handling
 * 
 * Features:
...
function doGet(e) {
  try {
    ...
    if (sheetName && !VALID_SHEETS.includes(sheetName)) {
      return createErrorResponse('Invalid sheet name: ' + sheetName, 400, callback);
    }
    ...
    switch (action) {
      case 'get':
        ...
        break;
      case 'search':
        result = handleSearchData(e.parameter);
        break;
      case 'count':
        result = handleCountData(e.parameter);
        break;
      case 'health':
        result = handleHealthCheck();
        break;
      default:
        result = createErrorResponse('Invalid action: ' + action, 400, callback);
    }
    ...
    return createErrorResponse('Server error: ' + error.message, 500, e.parameter.callback);
  }
}
...
/**
 * Create a success response with CORS support
 */
...
/**
 * Create an error response with CORS support
 */
function createErrorResponse(message, code = 400, callback = null) {
  const response = {
    success: false,
    error: {
      message: message,
